...
const getWpMedia = async () => {
  ...
  return getAllWordpressPages(mediaEndpoint).then((data) => data.map(({ id, guid, title }) => ({ id, url: guid.rendered, title: title.rendered })));
};
...
const getPosts = async (wpMedia) => {
  ...
  return posts.map(({ id, title, slug, categories, tags, author, featured_media, date, content, excerpt }) => {
    ...
    const recipeTitleText = $('#zlrecipe-title').text();
    // const recipeTitle = $('#zlrecipe-title').replaceWith(`<h2 class="recipe--title">${recipeTitleText}</h2>`);
    const recipeYield = $('#zlrecipe-yield span').text();
    
    // const recipeIngredientsText = $('#zlrecipe-ingredients-list').text();
    // const recipeIngredients = $('#zlrecipe-ingredients').replaceWith(`<h3 class="recipe--ingredients">${recipeIngredientsText}</h3>`);
    const ingredientsWrapper = $('<ul id="#zlrecipe-ingredients-wrapper"></ul>');
    const recipeIngredientsHTML = $('#zlrecipe-ingredients-list').wrap(ingredientsWrapper).parent().html();
    const recipeIngredients = normalizeBlock(htmlToBlocks(recipeIngredientsHTML));
    
    const instructionsWrapper = $('<ul id="#zlrecipe-instructions-wrapper"></ul>');
    const recipeInstructionsHTML = $('#zlrecipe-instructions-list').wrap(instructionsWrapper).parent().html();
    const recipeInstructions = normalizeBlock(htmlToBlocks(recipeInstructionsHTML));
    
    const recipeSummaryHTML = $('#zlrecipe-summary').html();
    const recipeSummaryBlock = normalizeBlock(htmlToBlocks(recipeSummaryHTML));
    ...
    const post = {
      ...
      body: parsedBody,
      recipeTitle: recipeTitleText,
      recipeYield: recipeYield,
      recipeDescription: recipeSummaryBlock.blocks,
      recipeIngredients: recipeIngredients.blocks,
      recipeDirections: recipeInstructions.blocks,
    };
    return post;
  });
};
