...
const getPosts = async (wpMedia) => {
  ...
  return posts.map(({ id, title, slug, categories, tags, author, featured_media, date, content, excerpt }) => {
    ...
    const recipeIngredientsText = $('#zlrecipe-ingredients').text();
    const recipeIngredients = $('#zlrecipe-ingredients').replaceWith(`<h3 class="recipe--ingredients">${recipeIngredientsText}</h3>`);
    
    const recipeInstructionsText = $('#zlrecipe-instructions').text();
    const recipeInstructions = $('#zlrecipe-instructions').replaceWith(`<h3 class="recipe--ingredients">${recipeInstructionsText}</h3>`);
    
    const recipeSummary = $('#zlrecipe-summary .summary').wrapInner('<em></em>');
    // console.log(recipeSummary.html());
